...
gse = read.csv("GSE92945_Fibroblast_RNAseq_counts.csv", header = TRUE, row.names = 1)
...
ens2symbol = AnnotationDbi::select(org.Hs.eg.db,keys = rest$X, 
                                    columns="SYMBOL",
                                    keytype="ENSEMBL")
ens2symbol <- as_tibble(ens2symbol)
view(ens2symbol)

rest <- inner_join(rest, ens2symbol, by=c("X"="ENSEMBL"))
rest
